...
function isAdmin(user) {
    ...
    const adminValues = ['ADMINISTRADOR', 'ADMIN', 'admin', 'Administrador', 'Administrator'];
    
    // Verificar type e role
    return adminValues.includes(userData.type) || adminValues.includes(userData.role);
}
...
function getStudents(schoolId = null, classId = null) {
    const students = JSON.parse(localStorage.getItem('students'));
    if (schoolId && classId) {
        return students.filter(s => s.schoolId === schoolId && s.classId === classId);
    } else if (schoolId) {
...
function linkExamToClass(examId, schoolId, grade, classId, shift) {
    ...
    const link = {
        id: exam.links.length > 0 ? Math.max(...exam.links.map(l => l.id)) + 1 : 1,
        schoolId,
        grade,
        classId,
        shift,
...
function initQuestionBank() {
    ...
    try {
        ...
        if (!Array.isArray(questions)) {
            console.warn('Banco de questões não está em formato de array, inicializando novamente');
            localStorage.setItem('questionBank', JSON.stringify([]));
        }
    } catch (error) {
...
function addQuestionToBank(question) {
    ...
    const existingQuestionIndex = questionBank.findIndex(q => 
    ...
    const newQuestion = {
        ...question,
        id: questionBank.length > 0 ? Math.max(...questionBank.map(q => q.id)) + 1 : 1,
...
function getQuestionBank(filters = {}) {
    ...
    if (filters.component) {
        questions = questions.filter(q => 
            q.metadata && q.metadata.curriculumComponent === filters.component
        );
    }
    
    if (filters.grade) {
        questions = questions.filter(q => 
            q.metadata && q.metadata.gradeLevel === filters.grade
        );
    }
    
    if (filters.difficulty) {
        questions = questions.filter(q => 
            q.metadata && q.metadata.difficultyLevel === filters.difficulty
        );
    }
    
    if (filters.educationLevel) {
        questions = questions.filter(q => 
            q.metadata && q.metadata.educationLevel === filters.educationLevel
        );
    }
    ...
    if (filters.searchTerm) {
        ...
        questions = questions.filter(q => 
            ...
            (q.options && q.options.some(opt => opt.text && opt.text.toLowerCase().includes(term))) ||
            (q.metadata && q.metadata.bnccCodes && q.metadata.bnccCodes.toLowerCase().includes(term))
        );
    }
...
function limitQuestionBankSize(maxQuestions = 500) {
    ...
    console.log(`Banco de questões limitado: ${questionsToRemove} questões antigas removidas.`);
    return questionBank;
}
...
function importQuestionBank(jsonFile, callback) {
    ...
    reader.onload = function(event) {
        try {
            ...
            if (!Array.isArray(questions)) {
                throw new Error('Formato inválido: o arquivo deve conter um array de questões');
            }
...
function showAlert(type, title, message, duration = 3000, callback = null) {
    ...
    if (!alertBox) {
        ...
        alertBox.className = 'fixed top-5 right-5 rounded-lg shadow-lg p-4 max-w-md z-50 transition-all duration-300 transform translate-x-full';
